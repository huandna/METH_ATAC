...
GENOME=./ref.fa
python ../get_genome.bed.py --input $GENOME --output ./xidian.bed
## make sliding windows: 500bp window size, 100bp step size
bedtools makewindows -b xidian.bed -w 500 -s 100 > Region.chr1.slide.xidian.bed
bedtools getfasta -tab -fi $GENOME -bed Region.chr1.slide.xidian.bed -fo Region.slide.xidian.input.txt
python ../pred_Human_chr.py --region ./Region.slide.xidian.input.txt --anno ../Female_mus_anno_5w.txt
...
#get bw
ln -s ./xidian.bed chrom.size
awk '{print $1"\t"$3}' xidian.bed > species_chrom_size.xls
